...
void sepia(int height, int width, RGBTRIPLE image[height][width]) // works
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j].rgbtRed = round(.393 * originalRed + .769 * originalGreen + .189 * originalBlue) > 255
                                      ? 255
                                      : round(.393 * originalRed + .769 * originalGreen + .189 * originalBlue);
            image[i][j].rgbtGreen = round(.349 * originalRed + .686 * originalGreen + .168 * originalBlue) > 255
                                        ? 255
                                        : round(.349 * originalRed + .686 * originalGreen + .168 * originalBlue);
            image[i][j].rgbtBlue = round(.272 * originalRed + .534 * originalGreen + .131 * originalBlue) > 255
                                       ? 255
...
void reflect(int height, int width, RGBTRIPLE image[height][width]) // works dont touch it
{
    for (int i = 0; i < height; i++)
    {
